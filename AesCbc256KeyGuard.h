...
public:
    ...
    void runTests(int threadCount, int testDuration)
    {
        ...
        // Generate the key
        status = NCryptCreatePersistedKey(g_hProvider, &g_hKey, NCRYPT_AES_ALGORITHM, NULL, 0, NCRYPT_USE_VIRTUAL_ISOLATION_FLAG);
        if (status != ERROR_SUCCESS)
        {
        ...
        status = NCryptSetProperty(g_hKey, NCRYPT_CHAINING_MODE_PROPERTY, (PBYTE)BCRYPT_CHAIN_MODE_CBC, sizeof(BCRYPT_CHAIN_MODE_CBC), 0);
        if (status != ERROR_SUCCESS)
        {
            wprintf(L"NCryptSetProperty failed with %x\n", status);
            NCryptFreeObject(g_hProvider);
        ...
        wprintf(L"\nCreated a persisted AES CBC 256 key!\n");

        // run sub-scenarios
        std::cout << "\n**** Executing Encrypt/Decrypt using AES CBC 256 ****" << std::endl;
        ...
        for (int idx = 0; idx < threadCount; ++idx)
        {
            threads[idx] = thread([&]() {
                ...
                while (std::chrono::steady_clock::now() - loop_start_time < std::chrono::seconds(testDuration))
                {
                    ...
                    status = NCryptEncrypt(g_hKey, plainText, plainTextSize, NULL, NULL, 0, &cipherTextSize, 0);
                    if (status != ERROR_SUCCESS)
                    {
                    ...
                    // Encrypt the data
                    status = NCryptEncrypt(g_hKey, plainText, plainTextSize, NULL, cipherText, cipherTextSize, &cipherTextSize, 0);
        ...
        for (int idx = 0; idx < threadCount; ++idx)
        {
            threads[idx] = thread([&]() {
                ...
                while (std::chrono::steady_clock::now() - loop_start_time < std::chrono::seconds(testDuration))
                {
                    // Decrypt 
                    start_time = std::chrono::high_resolution_clock::now();
                    status = NCryptDecrypt(g_hKey, cipherText, cipherTextSize, NULL, NULL, 0, &decryptedSize, 0);
                    if (status != ERROR_SUCCESS)
                    {
                    ...
                    status = NCryptDecrypt(g_hKey, cipherText, cipherTextSize, NULL, decryptedText, decryptedSize, &decryptedSize, 0);
                    if (status != ERROR_SUCCESS)
                    {
