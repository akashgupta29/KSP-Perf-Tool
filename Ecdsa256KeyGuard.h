...
public:
    ...
    void runTests(int threadCount, int testDuration)
    {
        ...
        // Generate the key
        status = NCryptCreatePersistedKey(g_hProvider, &g_hKey, NCRYPT_ECDSA_P256_ALGORITHM, NULL, 0, NCRYPT_USE_VIRTUAL_ISOLATION_FLAG);
        if (status != ERROR_SUCCESS)
        {
        ...
        wprintf(L"\nCreated a persisted ECDSA P256 key!\n");

        // run sub-scenarios
        std::cout << "\n**** Executing Sign operation using ECDSA P256 ****" << std::endl;
        TestSignEcdsaP256(threadCount, testDuration);
        std::cout << "*****************************************************\n" << std::endl;
...
private:
    void TestSignEcdsaP256(int threadCount, int testDuration)
    {
