...
public:
    ...
    void runTests(int threadCount, int testDuration)
    {
        ...
        // Generate the key
        status = NCryptCreatePersistedKey(g_hProvider, &g_hKey, NCRYPT_RSA_ALGORITHM, NULL, 0, NCRYPT_USE_VIRTUAL_ISOLATION_FLAG); // NCRYPT_USE_VIRTUAL_ISOLATION_FLAG
        if (status != ERROR_SUCCESS)
        {
...
private:
    ...
    void TestDecryptRsa2048(int threadCount, int testDuration)
    {
        ...
        // create a cipher text and use the same for decrypt operation
        SECURITY_STATUS status;
        BYTE plainText[128];
        DWORD plainTextSize = sizeof(plainText);
        generateRandomByteArray(128, plainText);
